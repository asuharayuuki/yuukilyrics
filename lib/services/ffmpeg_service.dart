...
class FfmpegService {
  ...
  Future<({int width, int height})?> getVideoResolution(
    String videoPath,
  ) async {
    if (isWindows) {
      final ffmpeg = await windowsFfmpegPath;
  ...
  Future<Uint8List> renderAssPreview({
    ...
    final assFile = File(p.join(tempDir.path, 'ass_style_preview_$id.ass'));
    final imageFile = File(p.join(tempDir.path, 'ass_style_preview_$id.png'));

    try {
      await assFile.writeAsString(assContent);
      final filter =
          'ass=f=${_escapeFilterPath(assFile.path)}:'
          'fontsdir=${_escapeFilterPath(fontSandboxDir)},'
          'crop=iw:ih/2:0:ih/2';
      final args = [
        '-y',
        '-loglevel',
        ...
        'lavfi',
        '-i',
        'color=c=0x101218:s=${width}x$height:d=4',
        '-ss',
        '3',
        '-vf',
        filter,
      ...
      if (isWindows) {
        ...
        if (returnCode?.isValueSuccess() != true) {
          throw Exception(await session.getOutput());
        }
      }
  ...
  Future<void> exportVideo({
    ...
    if (onEncoderDetected != null) {
      onEncoderDetected(
        '$encoder (${detected.isHardware ? "Hardware" : "Software"})',
      );
    }

    String finalAssPath = _escapeFilterPath(assPath);
    ...
    if (padVideo) {
      ...
      filterGraph =
          '[0:v]pad=width=\'ceil(max(iw\\,ih*16/9)/2)*2\':height=\'ceil(max(ih\\,iw*9/16)/2)*2\':x=-1:y=-1[padded];[padded]ass=f=$finalAssPath:fontsdir=$finalFontsDir,format=yuv420p[out]';
    } else {
      filterGraph =
          '[0:v]ass=f=$finalAssPath:fontsdir=$finalFontsDir,format=yuv420p[out]';
    }
    ...
    if (isWindows) {
      ...
      _activeProcess!.stderr.transform(SystemEncoding().decoder).listen((
        output,
      ) {
      ...
      await FFmpegKit.executeWithArgumentsAsync(
        ...
        (session) async {
          ...
          if (returnCode?.isValueSuccess() == true) {
            ...
            _mobileCompleter?.completeError(
              Exception(
                "FFmpeg encoding failed (code $returnCode).\nLog:\n$failLog",
              ),
            );
          }
        },
  ...
  static Future<void> cleanupTempFiles() async {
    try {
      ...
      final patterns = [
        ...
        RegExp(r'^ass_style_preview_\d+\.(ass|png)$'),
