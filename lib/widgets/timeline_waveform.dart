...
class _TimelineWaveformState extends State<TimelineWaveform> {
  ...
  Widget build(BuildContext context) {
    if (widget.waveformData == null) {
      return Center(
        child: Text(
          'Please open an audio/video file to view waveform.',
          style: TextStyle(
        ...
        return GestureDetector(
          ...
          onScaleUpdate: (details) {
            setState(() {
              ...
              if (details.scale != 1.0) {
                _pixelsPerSecond = (_basePixelsPerSecond * details.scale).clamp(10.0, 1000.0);
              }
              
              // Handle pan (drag)
              if (details.focalPointDelta.dx != 0) {
                // Drag left (negative delta) means moving forward in time
                final deltaMillis = -(details.focalPointDelta.dx / _pixelsPerSecond) * 1000;
                _dragPositionMillis += deltaMillis;
                _dragPositionMillis = _dragPositionMillis.clamp(0.0, widget.mediaPlayer.duration.inMilliseconds.toDouble());
                
                // Throttle seeking slightly or seek directly (media_kit handles frequent seeks ok when paused)
                widget.mediaPlayer.seek(Duration(milliseconds: _dragPositionMillis.toInt()));
              }
            });
          },
          onScaleEnd: (details) {
          ...
          child: AnimatedBuilder(
            ...
            builder: (context, child) {
              ...
              final double positionPixels = (currentPosition.inMilliseconds / 1000.0) * _pixelsPerSecond;
                      ...
                      child: ClipRect(
                        child: CustomPaint(
                          painter: WaveformPainter(
                            ...
                            waveColor: Theme.of(context).colorScheme.primary.withAlpha(150),
                          ),
                        ),
                      ),
                    ),
  ...
  String _formatDuration(Duration d) {
    ...
    String xx = threeDigits(d.inMilliseconds.remainder(1000)).substring(0, 2); // hundredths
    return "$mm:$ss:$xx";
  }
}
...
class WaveformPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final firstVisibleSample = ((-startX) / pixelPerSample).floor().clamp(0, samples.length);
    final lastVisibleSample = ((size.width - startX) / pixelPerSample).ceil().clamp(0, samples.length);
